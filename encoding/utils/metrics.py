...
class SegmentationMetric(object):
    ...
    def update(self, labels, preds):
        def evaluate_worker(self, label, pred):
            correct, labeled = np_batch_pix_accuracy(
                pred, label)
            inter, union = np_batch_intersection_union(
    ...
    def reset(self):
        ...
        return
##np version
def np_batch_pix_accuracy(output, target):
...
def np_batch_intersection_union(output, target, nclass):
...
# pytorch version
def batch_pix_accuracy(outputs, target):
    """PixAcc"""
...
def batch_intersection_union(outputs, target, nclass):
    """mIoU"""
    # inputs are numpy array, output 4D, target 3D
    outputs = list(outputs)
    output = outputs[0]
