...
class CitySegmentation(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #mask = Image.fromarray(self._class_to_index(np.array(Image.open(fname))).astype('int8'))
        mask_np = np.array(mask)
        mask_np = self.id2trainId(mask_np)
        mask = Image.fromarray(mask_np.astype('int8'))
    ...
    def _sync_transform(self, img, mask):
        ...
        crop_size_long = self.crop_size
        crop_size_short = int(crop_size_long * 0.5)
        w, h = img.size
        #if h > w:
        #    long_size_base = h
        #else:
        ...
        #long_size = random.randint(int(long_size_base*0.7), int(long_size_base*2.2))
        #short_size = random.randint(int(self.base_size*0.75), int(self.base_size*2.0))
        long_size = random.randint(int(self.base_size*0.5), int(self.base_size*2.2))
        if h > w:
            oh = long_size
            ow = int(1.0 * w * long_size / h + 0.5)
            short_size = ow
            crop_size_h = crop_size_long
            crop_size_w = crop_size_short
        else:
            ow = long_size
            oh = int(1.0 * h * long_size / w + 0.5)
            short_size = oh
            crop_size_w = crop_size_long
            crop_size_h = crop_size_short
    ...
    def make_pred(self, mask):
        ...
        mask = self.id2trainId(mask, reverse=True)
        return mask
